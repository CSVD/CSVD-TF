...
MODEL_NAME = "E:\models\codebert-base"
...
def main():
    ...
    df = pd.read_csv("VLC_code.csv", header=None)
    ...
    import pickle
    f = open('model_VLC/VLC_code_vector_whitening.pkl', 'wb')
    pickle.dump(vecs_func_body, f)

    f.close()
    f = open('model_VLC/VLC_kernel.pkl', 'wb')
    pickle.dump(kernel, f)
    f.close()
    f = open('model_VLC/VLC_bias.pkl', 'wb')
    pickle.dump(bias, f)
    f.close()
